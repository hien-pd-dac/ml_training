...
def get_tf_idf(data_path):
	...
	with open('..datasets/20news-bydate/words_idfs.txt') as f:
		words_idf = [(line.split('<fff>')[0], float(line.split('<fff>')[1]))
	...
	with open(data_path) as f:
		...
		for document in documents:
			...
			spares_rep = ' '.join(words_tfidfs_normalized)
			data_tf_idf.append((label, doc_id, spares_rep))
